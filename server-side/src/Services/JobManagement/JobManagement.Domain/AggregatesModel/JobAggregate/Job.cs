...
namespace JobManagement.Domain.AggregatesModel.JobAggregate
{
    public class Job : EventSourcedAggregate
    {
        ...
        {
            var @event = new JobUpdated(Id, title, description, EvaluateCredits(experienceLevel), experienceLevel, payment, questions, professionId);
            Causes(@event);
        }
        ...
        public Result AddProposal(Proposal proposal)
        {
            ...
            var @event = new ProposalCreated(Id, proposal);
            Causes(@event);
